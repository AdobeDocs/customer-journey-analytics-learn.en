---
title: Add [!UICONTROL Scatter] Visualizations in [!DNL Analysis Workspace] Projects
description: Learn how to use the [!UICONTROL scatter] visualization in [!DNL Analysis Workspace] projects in [!DNL Customer Journey Analytics].
feature: Visualizations
kt: 13428
role: User
level: Beginner
last-substantial-update: 2026-02-17
exl-id: 2733a815-c030-4fad-81d9-3f38fb1e7378
---
# Use [!UICONTROL scatter] visualizations in [!DNL Analysis Workspace] projects in [!DNL Customer Journey Analytics]

Learn how to use the [!UICONTROL scatter] visualization in [!DNL Analysis Workspace] projects in [!DNL Customer Journey Analytics]. This visualization shows the relationship between dimension items and up to three metrics.

>[!VIDEO](https://video.tv.adobe.com/v/3479958/?quality=12&learn=on)

For more information about [!UICONTROL scatter] visualizations, visit the [documentation](https://experienceleague.adobe.com/docs/analytics-platform/using/cja-workspace/visualizations/scatterplot.html){target="_blank"}.
